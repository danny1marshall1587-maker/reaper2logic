#!/usr/bin/env perl
# REAPER (.rpp) <-> Logic Pro (.logicx) Converter Engine in Perl
# Direct Native Logic Pro Bundle Builder - Mimics native Logic Pro documents directly without FCPXML import
...
package_rpp_to_direct_logicx($rpp_file, $audio_dir, $out_bundle, $filename);

sub package_rpp_to_direct_logicx {
    ...
    # Also scan audio directory recursively for any additional audio files (.wav, .aif, .mp3, .flac, .m4a)
    if (-d $proj_dir) {
        scan_and_copy_audio($proj_dir, $media_dir, \%copied_map, \$copied_count);
    }

    # 5. Generate ProjectInformation.plist so Logic Pro opens Alternative 000 natively on double-click
    ...
    # 6. Generate FCPXML inside bundle as backup
    ...
    print "🎉 Successfully created direct native Logic Pro bundle (.logicx): '$out_path'\n";
